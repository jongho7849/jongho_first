...
UICHANG_MIDDLES = [
    ...
    "신월중학교", "창덕중학교", "창북중학교", "감계중학교", "창원대산중학교",
...
SCHOOL_DATA = {
    ...
    "북면고등학교": {
        "gender": "공학",
        "zone": "의창",
        "feature": "의창구 북면 지역의 일반고로, 지역 학생 중심의 안정적인 공학 일반고입니다.",
        "pros": "내신 경쟁이 비교적 완만하여 성적을 다시 정비하거나 기초를 다지기에 유리합니다.",
        "cons": "상위권 학생을 위한 고급 선택과목·학종 특화 프로그램의 다양성은 상대적으로 제한적입니다.",
        "credit": "기초 중심 교과 위주의 학점제 운영으로, 필수·기본 교과 중심 편성이 특징입니다.",
        "hakjong": "학종보다는 교과·정시 비중이 큰 진학 구조를 보이며, 중위권 학생에게 적합합니다.",
        "seteuk": "수업 참여·과제 성실성 위주의 기록이 많고, 심화 탐구 내용은 학생 주도 여부에 따라 달라집니다.",
    },
    "창원토월고등학교": {
        "gender": "공학",
        "zone": "의창",
...
# ======================================
# 1-2. 고교별 인문/자연/종합·진학 레벨 설정 (상담용 내부 기준)
# ======================================
...
def get_school_eval_tags(school_name: str):
    """각 고등학교에 대해 인문/자연/종합 랭킹과 대입 진학 경향 문구를 생성"""
    # 인문계열 평가
    if school_name in HUMAN_TOP3:
        human_tag = "인문계열 최상위권 (상담 기준 Top3)"
    elif school_name in HUMAN_HIGH:
        human_tag = "인문계열 상위권 (중상위권 이상)"
    else:
        human_tag = "인문계열은 평균적인 수준(학교 내 학생 구성·프로그램 기준)"

    # 자연계열 평가
    if school_name in SCI_TOP3:
        science_tag = "자연계열 최상위권 (상담 기준 Top3)"
    elif school_name in SCI_HIGH:
        science_tag = "자연계열 상위권 (중상위권 이상)"
    else:
        science_tag = "자연계열은 평균적인 수준(수학·과학 선택과목 및 실적 기준)"

    # 종합(학종+수능+학교 분위기) 평가
    if school_name in OVERALL_TOP5:
        overall_tag = "종합 학업·학종 경쟁력 Top5권 학교(상담 기준)"
    else:
        overall_tag = "종합 경쟁력은 중위권 수준(내신·비교과·대입실적 종합)"

    # 대입 진학 경향
    level = ADMISSION_LEVEL.get(school_name, "중위권")
    if level == "최상위권":
        admission_text = (
            "서울·수도권 상위권 대학과 거점국립대(경상국립대·부산대 등) 합격자가 꾸준히 나오며, "
            "의치한·상경·공대 등 최상위 계열 진학도 일정 비율 유지되는 편입니다."
        )
    elif level == "중상위권":
        admission_text = (
            "경상국립대·부산대 등 거점국립대와 수도권 중상위권 대학, "
            "지방 4년제가 고르게 분포하는 구조로, 상위·중위권 학생 모두 진학 폭이 넓은 편입니다."
        )
    elif level == "중위권":
        admission_text = (
            "지방 4년제 대학과 일부 거점국립대, 수도권 대학 합격이 섞여 있으며, "
            "전문대 진학 비율도 어느 정도 존재하는 전형적인 중위권 일반고 진학 패턴입니다."
        )
    else:  # 안정형
        admission_text = (
            "지역 4년제·전문대 진학 비중이 높고, 상위권 학생 일부가 거점국립대 및 수도권 대학에 "
            "진학하는 '안정형' 진학 패턴을 보이는 학교입니다."
        )

    return human_tag, science_tag, overall_tag, admission_text
...
def main():
    ...
    if st.button("1지망 ~ 5지망 추천 보기"):
        ...
        for i, school in enumerate(recs, start=1):
            ...
            # 인문/자연/종합 랭킹 + 대입 진학 경향
            human_tag, science_tag, overall_tag, admission_text = get_school_eval_tags(school)
            st.markdown(f"- **인문계열 기준 상대 위치**: {human_tag}")
            st.markdown(f"- **자연계열 기준 상대 위치**: {science_tag}")
            st.markdown(f"- **종합(학종+수능) 경쟁력 평가**: {overall_tag}")
            st.markdown(f"- **대입 진학 경향(상담 기준)**: {admission_text}")

            st.markdown("---")

        st.caption("※ 위 평가는 실제 배정·진학 결과를 보장하는 통계가 아니라, 학교별 일반적인 경향을 바탕으로 한 상담 참고용입니다.")
